...
module amcore_68k_baseline(
 ...
 typedef enum logic[4:0]{S_RESET_SSP_HI,S_RESET_SSP_LO,S_RESET_PC_HI,S_RESET_PC_LO,S_FETCH,S_EXEC,S_IMM_SR,S_BRANCH_EXT,S_BSR_PUSH_LO,S_BSR_PUSH_HI,S_RTS_POP_HI,S_RTS_POP_LO,S_EXC_PUSH_PC_LO,S_EXC_PUSH_PC_HI,S_EXC_PUSH_SR,S_EXC_VEC_HI,S_EXC_VEC_LO,S_RTE_POP_SR,S_RTE_POP_PC_HI,S_RTE_POP_PC_LO,S_MEM_RD_HI,S_MEM_RD_LO,S_MEM_WR_HI,S_MEM_WR_LO,S_HALT,S_DBCC_EXT,S_JUMP_EXT_W,S_JUMP_EXT_HI,S_JUMP_EXT_LO}state_t;
 state_t state; logic[31:0]dreg[0:7],areg[0:7],usp,ssp; logic[15:0]ir,reset_hi,vector_hi,rte_pc_hi,rts_pc_hi,rte_sr,jump_hi; logic[31:0]exception_pc;logic[15:0]exception_sr;logic[3:0]quick_value;logic irq_pending;logic[31:0]alu_result;logic[32:0]alu_wide;logic alu_x,alu_n,alu_z,alu_v,alu_c;logic[31:0]mem_ea,mem_value;logic[2:0]mem_dreg,mem_areg;logic[1:0]mem_update;logic branch_is_bsr,branch_take,jump_is_jsr;logic[31:0]branch_return;integer i;
 ...
 always_ff@(posedge clk)begin
  if(!reset_n)begin state<=S_RESET_SSP_HI;reset_hi<=0;vector_hi<=0;rte_pc_hi<=0;rts_pc_hi<=0;rte_sr<=16'h2700;exception_pc<=0;exception_sr<=0;pc<=0;sr<=16'h2700;ir<=0;halted<=0;exception<=0;exception_vector<=0;usp<=0;ssp<=0;mem_ea<=0;mem_value<=0;mem_dreg<=0;mem_areg<=0;mem_update<=0;branch_is_bsr<=0;branch_take<=0;branch_return<=0;jump_hi<=0;jump_is_jsr<=0;for(i=0;i<8;i=i+1)begin dreg[i]<=0;areg[i]<=0;end end
   ...
   S_EXEC:begin
    ...
    else if(ir==16'h4EF9||ir==16'h4EB9)begin jump_is_jsr<=(ir==16'h4EB9);state<=S_JUMP_EXT_HI;end
   ...
   S_JUMP_EXT_W:if(ack)begin if(jump_is_jsr)begin branch_return<=pc+4;pc<={{16{data_in[15]}},data_in};state<=S_BSR_PUSH_LO;end else begin pc<={{16{data_in[15]}},data_in};state<=S_FETCH;end end
