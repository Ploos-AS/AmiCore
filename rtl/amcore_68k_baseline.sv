...
module amcore_68k_baseline(
 ...
 typedef enum logic[5:0]{S_RESET_SSP_HI,S_RESET_SSP_LO,S_RESET_PC_HI,S_RESET_PC_LO,S_FETCH,S_EXEC,S_IMM_SR,S_BRANCH_EXT,S_BSR_PUSH_LO,S_BSR_PUSH_HI,S_RTS_POP_HI,S_RTS_POP_LO,S_EXC_PUSH_PC_LO,S_EXC_PUSH_PC_HI,S_EXC_PUSH_SR,S_EXC_VEC_HI,S_EXC_VEC_LO,S_RTE_POP_SR,S_RTE_POP_PC_HI,S_RTE_POP_PC_LO,S_MEM_RD_HI,S_MEM_RD_LO,S_MEM_WR_HI,S_MEM_WR_LO,S_HALT,S_DBCC_EXT,S_JUMP_EXT_W,S_JUMP_EXT_HI,S_JUMP_EXT_LO,S_EA_EXT_W,S_EA_EXT_HI,S_EA_EXT_LO,S_MOVE_EXT,S_MOVE_ABS_HI,S_MOVE_ABS_LO,S_MOVE_DST_EXT,S_MOVE_DST_ABS_HI,S_MOVE_DST_ABS_LO}state_t;
 state_t state; logic[31:0]dreg[0:7],areg[0:7],usp,ssp; logic[15:0]ir,reset_hi,vector_hi,rte_pc_hi,rts_pc_hi,rte_sr,jump_hi,ea_hi; logic[31:0]exception_pc;logic[15:0]exception_sr;logic[3:0]quick_value;logic irq_pending;logic[31:0]alu_result;logic[32:0]alu_wide;logic alu_x,alu_n,alu_z,alu_v,alu_c;logic[31:0]mem_ea,mem_value,ea_base;logic[2:0]mem_dreg,mem_areg,mem_dest_areg,ea_dest_areg;logic[1:0]mem_update;logic branch_is_bsr,branch_take,jump_is_jsr,jump_is_pc_relative,jump_is_indexed,ea_is_pea,ea_is_absw,ea_is_indexed;logic move_ext_indexed,move_ext_pc_relative,move_ext_absolute,mem_to_areg;logic[31:0]branch_return;logic[31:0]indexed_value;logic[31:0]move_index_value;integer i;
 ...
 always_comb begin
  indexed_value=data_in[15]?areg[data_in[14:12]]:dreg[data_in[14:12]];move_index_value=data_in[15]?areg[data_in[14:12]]:dreg[data_in[14:12]];
 ...
 always_ff@(posedge clk)begin
  if(!reset_n)begin state<=S_RESET_SSP_HI;reset_hi<=0;vector_hi<=0;rte_pc_hi<=0;rts_pc_hi<=0;rte_sr<=16'h2700;exception_pc<=0;exception_sr<=0;pc<=0;sr<=16'h2700;ir<=0;halted<=0;exception<=0;exception_vector<=0;usp<=0;ssp<=0;mem_ea<=0;mem_value<=0;mem_dreg<=0;mem_areg<=0;mem_dest_areg<=0;mem_update<=0;mem_to_areg<=0;branch_is_bsr<=0;branch_take<=0;branch_return<=0;jump_hi<=0;jump_is_jsr<=0;jump_is_pc_relative<=0;jump_is_indexed<=0;ea_hi<=0;ea_base<=0;ea_dest_areg<=0;ea_is_pea<=0;ea_is_absw<=0;ea_is_indexed<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=0;for(i=0;i<8;i=i+1)begin dreg[i]<=0;areg[i]<=0;end end
   ...
   S_EXEC:begin
    ...
    else if(ir[15:12]==2&&ir[8:6]==1&&ir[5:3]==7&&ir[2:0]==0)begin mem_dest_areg<=ir[11:9];mem_to_areg<=1;mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=1;state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==1&&ir[5:3]==7&&ir[2:0]==1)begin mem_dest_areg<=ir[11:9];mem_to_areg<=1;mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=1;state<=S_MOVE_ABS_HI;end
    else if(ir[15:12]==2&&ir[8:6]==1&&(ir[5:3]==5||ir[5:3]==6))begin mem_dest_areg<=ir[11:9];mem_to_areg<=1;mem_areg<=ir[2:0];mem_update<=0;move_ext_indexed<=(ir[5:3]==6);move_ext_pc_relative<=0;move_ext_absolute<=0;ea_base<=areg[ir[2:0]];state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==1&&ir[5:3]==7&&(ir[2:0]==2||ir[2:0]==3))begin mem_dest_areg<=ir[11:9];mem_to_areg<=1;mem_update<=0;move_ext_indexed<=(ir[2:0]==3);move_ext_pc_relative<=1;move_ext_absolute<=0;ea_base<=pc+2;state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==1&&(ir[5:3]==2||ir[5:3]==3||ir[5:3]==4))begin mem_dest_areg<=ir[11:9];mem_to_areg<=1;mem_areg<=ir[2:0];mem_update<=0;if(ir[5:3]==4)begin mem_ea<=areg[ir[2:0]]-4;areg[ir[2:0]]<=areg[ir[2:0]]-4;mem_update<=2;end else begin mem_ea<=areg[ir[2:0]];if(ir[5:3]==3)mem_update<=1;end state<=S_MEM_RD_HI;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==0)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=1;state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==1)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=1;state<=S_MOVE_ABS_HI;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==5)begin mem_dreg<=ir[11:9];mem_areg<=ir[2:0];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;ea_base<=areg[ir[2:0]];state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==6)begin mem_dreg<=ir[11:9];mem_areg<=ir[2:0];mem_update<=0;move_ext_indexed<=1;move_ext_pc_relative<=0;ea_base<=areg[ir[2:0]];state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==2)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=1;ea_base<=pc+2;state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==3)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=1;move_ext_pc_relative<=1;ea_base<=pc+2;state<=S_MOVE_EXT;end
    ...
    else if(ir[15:12]==2&&ir[5:3]==0&&(ir[8:6]==5||ir[8:6]==6))begin mem_value<=dreg[ir[2:0]];mem_areg<=ir[11:9];mem_update<=0;sr[3]<=dreg[ir[2:0]][31];sr[2]<=(dreg[ir[2:0]]==0);sr[1:0]<=0;ea_base<=areg[ir[11:9]];move_ext_indexed<=(ir[8:6]==6);state<=S_MOVE_DST_EXT;end
    else if(ir[15:12]==2&&ir[5:3]==0&&ir[8:6]==7&&ir[11:9]==0)begin mem_value<=dreg[ir[2:0]];mem_update<=0;sr[3]<=dreg[ir[2:0]][31];sr[2]<=(dreg[ir[2:0]]==0);sr[1:0]<=0;state<=S_MOVE_DST_EXT;end
    else if(ir[15:12]==2&&ir[5:3]==0&&ir[8:6]==7&&ir[11:9]==1)begin mem_value<=dreg[ir[2:0]];mem_update<=0;sr[3]<=dreg[ir[2:0]][31];sr[2]<=(dreg[ir[2:0]]==0);sr[1:0]<=0;state<=S_MOVE_DST_ABS_HI;end
   ...
   S_MOVE_DST_EXT:if(ack)begin if(ir[8:6]==7)mem_ea<={{16{data_in[15]}},data_in};else if(move_ext_indexed)mem_ea<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else mem_ea<=ea_base+{{16{data_in[15]}},data_in};pc<=pc+2;state<=S_MEM_WR_HI;end
   S_MOVE_DST_ABS_HI:if(ack)begin ea_hi<=data_in;state<=S_MOVE_DST_ABS_LO;end
   S_MOVE_DST_ABS_LO:if(ack)begin mem_ea<={ea_hi,data_in};pc<=pc+4;state<=S_MEM_WR_HI;end
   S_MOVE_EXT:if(ack)begin if(move_ext_absolute)mem_ea<={{16{data_in[15]}},data_in};else if(move_ext_indexed)mem_ea<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?move_index_value:{{16{move_index_value[15]}},move_index_value[15:0]});else mem_ea<=ea_base+{{16{data_in[15]}},data_in};pc<=pc+2;state<=S_MEM_RD_HI;end
   S_MOVE_ABS_HI:if(ack)begin ea_hi<=data_in;state<=S_MOVE_ABS_LO;end
   S_MOVE_ABS_LO:if(ack)begin mem_ea<={ea_hi,data_in};pc<=pc+4;state<=S_MEM_RD_HI;end
   S_EA_EXT_W:if(ack)begin if(ea_is_absw)branch_return<={{16{data_in[15]}},data_in};else if(ea_is_indexed)branch_return<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else branch_return<=ea_base+{{16{data_in[15]}},data_in};if(ea_is_pea)begin pc<=pc+4;state<=S_BSR_PUSH_LO;end else begin if(ea_is_absw)areg[ea_dest_areg]<={{16{data_in[15]}},data_in};else if(ea_is_indexed)areg[ea_dest_areg]<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else areg[ea_dest_areg]<=ea_base+{{16{data_in[15]}},data_in};pc<=pc+4;state<=S_FETCH;end end
   ...
   S_JUMP_EXT_W:if(ack)begin if(jump_is_jsr)begin branch_return<=pc+4;if(jump_is_indexed)pc<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else if(jump_is_pc_relative)pc<=pc+2+{{16{data_in[15]}},data_in};else pc<={{16{data_in[15]}},data_in};state<=S_BSR_PUSH_LO;end else begin if(jump_is_indexed)pc<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else if(jump_is_pc_relative)pc<=pc+2+{{16{data_in[15]}},data_in};else pc<={{16{data_in[15]}},data_in};state<=S_FETCH;end end
   ...
   S_MEM_RD_HI:if(ack)begin mem_value[31:16]<=data_in;state<=S_MEM_RD_LO;end S_MEM_RD_LO:if(ack)begin mem_value[15:0]<=data_in;if(mem_to_areg)begin areg[mem_dest_areg]<={mem_value[31:16],data_in};end else begin dreg[mem_dreg]<={mem_value[31:16],data_in};sr[3]<=mem_value[31];sr[2]<=({mem_value[31:16],data_in}==0);sr[1:0]<=0;end if(mem_update==1)areg[mem_areg]<=areg[mem_areg]+4;if(move_ext_indexed||move_ext_pc_relative||ir[5:3]==5)pc<=pc+2;else pc<=pc+2;move_ext_indexed<=0;move_ext_pc_relative<=0;move_ext_absolute<=0;mem_to_areg<=0;state<=S_FETCH;end S_MEM_WR_HI:if(ack)state<=S_MEM_WR_LO;S_MEM_WR_LO:if(ack)begin if(mem_update==1)areg[mem_areg]<=areg[mem_areg]+4;pc<=pc+2;state<=S_FETCH;end
