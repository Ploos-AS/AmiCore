 ...
 always_ff@(posedge clk)begin
  if(!reset_n)begin state<=S_RESET_SSP_HI;reset_hi<=0;vector_hi<=0;rte_pc_hi<=0;rts_pc_hi<=0;rte_sr<=16'h2700;exception_pc<=0;exception_sr<=0;pc<=0;sr<=16'h2700;ir<=0;halted<=0;exception<=0;exception_vector<=0;usp<=0;ssp<=0;mem_ea<=0;mem_value<=0;mem_dreg<=0;mem_areg<=0;mem_update<=0;branch_is_bsr<=0;branch_take<=0;branch_return<=0;jump_hi<=0;jump_is_jsr<=0;jump_is_pc_relative<=0;jump_is_indexed<=0;ea_hi<=0;ea_base<=0;ea_dest_areg<=0;ea_is_pea<=0;ea_is_absw<=0;ea_is_indexed<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;for(i=0;i<8;i=i+1)begin dreg[i]<=0;areg[i]<=0;end end
   ...
   S_EXEC:begin
    ...
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==0)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;state<=S_MOVE_EXT;end
    else if(ir[15:12]==2&&ir[8:6]==0&&ir[5:3]==7&&ir[2:0]==1)begin mem_dreg<=ir[11:9];mem_update<=0;move_ext_indexed<=0;move_ext_pc_relative<=0;state<=S_MOVE_ABS_HI;end
   ...
   S_MOVE_EXT:if(ack)begin if(ir[5:3]==7&&ir[2:0]==0)mem_ea<={{16{data_in[15]}},data_in};else if(move_ext_indexed)mem_ea<=ea_base+{{24{data_in[7]}},data_in[7:0]}+(data_in[11]?indexed_value:{{16{indexed_value[15]}},indexed_value[15:0]});else mem_ea<=ea_base+{{16{data_in[15]}},data_in};pc<=pc+2;state<=S_MEM_RD_HI;end
