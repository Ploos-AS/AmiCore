...
module tb_m1_2_cpu;
    ...
    initial begin
        ...
        mem[20] = 16'hFFFF;
        #20 reset_n = 1;
        wait (pc == 32'h00000022); @(posedge clk); if (d0 !== 32'd5) $fatal(1, "MOVEQ failed: D0=%h", d0);
        wait (pc == 32'h00000024); @(posedge clk); if (d0 !== 32'd8) $fatal(1, "ADDQ failed: D0=%h", d0);
        wait (pc == 32'h00000026); @(posedge clk); if (d0 !== 32'd7) $fatal(1, "SUBQ failed: D0=%h", d0);
        wait (pc == 32'h00000028); @(posedge clk); if (d0 !== 32'd0) $fatal(1, "CLR failed: D0=%h", d0);
        wait (exception == 1'b1);
        if (exception_vector !== 8'd4) $fatal(1, "wrong exception vector: %0d", exception_vector);
