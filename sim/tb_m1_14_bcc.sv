...
module tb_m1_14_bcc;
 ...
 always_comb begin case(address)
  ...
  // Z=1: BGT.w not taken must consume extension. MOVEQ #7,D7 proves the
  // fall-through path, then MOVEQ #0,D0 deliberately restores Z=1 before
  // BLE.w. In this core's branch state pc still denotes the opcode address,
  // so BLE.w +6 targets $132 and skips MOVEQ #9,D0 plus the NOP at $130.
  32'h120:data_in=16'h7000; 32'h122:data_in=16'h6e00; 32'h124:data_in=16'h0004; 32'h126:data_in=16'h7e07; 32'h128:data_in=16'h7000; 32'h12a:data_in=16'h6f00; 32'h12c:data_in=16'h0006; 32'h12e:data_in=16'h7009; 32'h130:data_in=16'h4e71; 32'h132:data_in=16'h60fe;
  default:data_in=16'h4e71;endcase end
 initial begin repeat(2)@(posedge clk);#1;reset_n=1;cycles=0;while(pc!=32'h132&&cycles<400)begin @(posedge clk);#1;cycles=cycles+1;end
